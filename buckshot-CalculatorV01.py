...
def parte1():
        ...
        while arte1_is_running:
                ...
                print(f"Probability is {Fraction(BLANK, total_ammo)} blanks & {Fraction(FULL, total_ammo)}")
                # parte che aggiorna la funzione above
                # end loop
                arte1_is_running = False
        ...
        while anyNEWS:
                ...
                if condizione == 'y':
                        ...
                        UPDATE = input('どのcontidion?(shot/soda[S] OR else): ').strip().upper()
                        if UPDATE == 'S':
                                anyNEWS = False

                                Used_bullet = input('B/F?: ').upper()
                                if Used_bullet == 'B':
                                        BLANK -= 1
                                elif Used_bullet == 'F':
                                        FULL -= 1
                                ...
                                total_ammo = BLANK + FULL
                                        # il pezzo ridondante inizia qui
                                print(f"there are {total_ammo} bullets in this cartridge, {BLANK} blanks & {FULL} fulls left")
                                ...
                                print(f"Probability is {Fraction(BLANK, total_ammo)} blanks & {Fraction(FULL, total_ammo)}")
                                        # ripeto il pezzo pke non ho voja di metterlo in hna funzione fine pezzo ridondante
